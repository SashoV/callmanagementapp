    ...
    <div class="col-md-6 col-md-offset-3">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form method="POST" action="{{ route('storeCall') }}">
                    @csrf
                    <div class="form-group @error('user') has-error @enderror">
                        ...
                        <select class="form-control" name="user" id="user">
                            ...
                            <option value="{{ $user }}" @if ($user==old('user')) {{ "selected" }} @endif>
                                {{ $user }}</option>
                            @endforeach
                        </select>
                        @error('user')
                    ...
                    <div class="form-group @error('client') has-error @enderror">
                        ...
                        <select class="form-control" name="client" id="client">
                            ...
                            <option value="{{ $client }}" @if ($client==old('client')) {{ "selected" }} @endif>
                                {{ $client }}</option>
                            @endforeach
                        </select>
                        @error('client')
                    ...
                    <div class="form-group @error('client_type') has-error @enderror">
                        ...
                        <input type="text" class="form-control" id="client_type" placeholder="Client Type"
                            name="client_type" value="{{ old('client_type') }}">
                        @error('client_type')
                        <span class="red">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group @error('date') has-error @enderror">
                        ...
                        <input type="text" class="form-control" id="date" name="date" value="{{ old('date')}}"
                            maxlength="20" placeholder="Select Date and Time">
                        @error('date')
                        <span class="red">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group @error('type_of_call') has-error @enderror">
                        ...
                        <select class="form-control" name="type_of_call" id="type_of_call">
                            ...
                            <option value="{{ $type_of_call }}" @if ($type_of_call==old('type_of_call'))
                                {{ "selected" }} @endif>
                                {{ $type_of_call }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group @error('external_call_score') has-error @enderror">
                        ...
                        <input type="number" class="form-control" id="external_call_score" name="external_call_score"
                            value="{{old('external_call_score')}}" placeholder="External Call Score">
